<template>
  <div class="box">
    <div class="img">
        <img src="/src/assets/login/banner.jpg" alt="">
    </div>
    <div class="two">
      <!-- 导航 -->
      <div class="TwoNavigation">
        <div class="loge">
          <img src="/src/assets/index/TopNavigation/logo.jpg" alt="">
        </div>
        <div class="right">
          <ul>
            <li><a href="">用户协议</a></li>
            <li><a href="">隐私政策</a></li>
            <li><a href="">帮助中心</a></li>
            <li>|</li>
            <li><a href="">中文(简体)</a>
              <!-- 二级导航 -->
              <ul class="sub-nav">
                <li><a href="">中文(繁体)</a></li>
                <li><a href="">English</a></li>
                <li><a href="">བོད་སྐད</a></li>
                <li><a href="">ئۇيغۇرچە</a></li>
              </ul>
              <!-- 二级导航 end-->
            </li>
          </ul>
        </div>
      </div>
      <!-- 导航 end-->
      <!-- 登录 -->
      <div class="form">
        <div class="form-center">
          <!-- 标题 -->
          <!-- 登陆路由 -->
          <router-link to="/login" class="title" style="color: red;">登录</router-link>
          &nbsp;
          <!-- 注册路由 -->
          <router-link to="/register" class="title">注册</router-link>
          <!-- 输入框 -->
          <input type="text" class="input" placeholder="请输入用户名" v-model="username">
          <input type="password" class="input" placeholder="请输入密码" v-model="password">
          <!-- 条款 -->
          <div class="tk">
            <input type="radio" class="radio">
            <span>已阅读并同意小米账号用户协议和隐私政策</span>
          </div>
          <!-- 确认 -->
          <input type="button" value="登陆" class="button" @click="registerUser">
          <!-- 链接 -->
          <div class="lj">
            <a href="">忘记密码</a>
            <a href="">手机号登录</a>
          </div>
          <!-- 其他方式登陆 -->
          <div class="qtfsdl">
            <span>其他方式登陆</span>
            <div>
              <a href=""></a>
              <a href=""><svg viewBox="0 0 46 46" xmlns="http://www.w3.org/2000/svg" aria-label="alipay"><path d="M10.35 25.898c-.543.439-1.125 1.076-1.294 1.886-.23 1.109-.048 2.496 1.021 3.583 1.296 1.32 3.263 1.68 4.112 1.744 2.31.165 4.77-.978 6.626-2.285a14.633 14.633 0 003.162-3.133c-2.667-1.376-5.996-2.898-9.555-2.75-1.818.076-3.12.454-4.072.956zm33.821 6.09A22.878 22.878 0 0046.001 23C46 10.319 35.683 0 23 0S0 10.319 0 23c0 12.684 10.315 23 23 23 7.654 0 14.441-3.761 18.624-9.53-4.82-2.4-9.646-4.785-14.48-7.156-1.916 2.184-4.746 4.372-7.943 5.324-2.01.598-3.822.825-5.715.438-1.875-.383-3.257-1.261-4.062-2.142-.41-.449-.88-1.02-1.222-1.699.032.086.055.137.055.137s-.196-.337-.346-.876a4.202 4.202 0 01-.16-1.401c-.02-.362.005-.726.077-1.081.187-.91.575-1.967 1.58-2.952 2.203-2.157 5.154-2.274 6.684-2.265 2.265.014 6.201 1.005 9.515 2.178.918-1.957 1.507-4.048 1.886-5.44H13.717v-1.49h7.075v-2.982h-8.567v-1.489h8.565v-2.979c0-.41.081-.745.745-.745h3.352v3.724H34.2v1.49h-9.312v2.98h7.45s-.748 4.17-3.088 8.28c5.192 1.854 12.494 4.71 14.92 5.664z" fill="#1989fa"></path></svg></a>
              <a href=""><svg viewBox="0 0 46 46" xmlns="http://www.w3.org/2000/svg" aria-label="wechat"><g fill="#50b674"><path d="M14.345 17.311a1.505 1.505 0 103.01 0 1.505 1.505 0 00-3.01 0zm6.74-.04a1.505 1.505 0 103.01 0 1.505 1.505 0 00-3.01 0zm3.756 6.732a1.045 1.045 0 102.09 0 1.045 1.045 0 00-2.09 0zm5.31.084a1.045 1.045 0 102.092 0 1.045 1.045 0 00-2.091 0z"></path><path d="M23 0A23.001 23.001 0 006.736 39.263 23 23 0 0046 22.999 22.984 22.984 0 0023 0zm-3.93 28.311c-1.212 0-2.175-.25-3.387-.501l-3.387 1.673.961-2.885c-2.426-1.674-3.849-3.849-3.849-6.525 0-4.601 4.35-8.197 9.662-8.197 4.725 0 8.906 2.886 9.743 6.774a6.763 6.763 0 00-.921-.042c-4.601 0-8.197 3.429-8.197 7.652.01.676.11 1.348.293 1.998-.335 0-.628.042-.92.042zm14.135 3.387l.71 2.426-2.634-1.463c-.961.25-1.924.502-2.885.502-4.601 0-8.199-3.136-8.199-6.974 0-3.837 3.596-6.984 8.196-6.984 4.35 0 8.197 3.136 8.197 6.984 0 2.133-1.423 4.057-3.387 5.52z"></path></g></svg></a>
              <a href=""><svg viewBox="0 0 46 46" xmlns="http://www.w3.org/2000/svg" aria-label="qq"><path d="M23 0C10.31 0 0 10.31 0 23s10.31 23 23 23 23-10.31 23-23S35.69 0 23 0zm10.363 29.926c-.053.053-.106.053-.159.106-.634-.159-1.374-.74-1.692-1.216-.158-.159-.211-.423-.475-.53 0 .953-.582 1.852-1.005 2.486-.159.211-.635.582-.635.793.16.106.37.159.582.212.476.211 1.163.581 1.428 1.004.105.159.264.317.317.476.74 1.745-1.64 2.168-2.855 2.432s-3.49-.476-4.124-.846c-.37-.264-.74-.846-1.322-.846-.159.106-.635.053-.9.053-.74 1.48-4.6 2.168-6.661 1.375-.635-.212-1.956-.793-1.692-1.798.211-.846.899-1.322 1.586-1.639.264-.159.846-.211 1.005-.476-.582-.423-1.11-1.269-1.375-1.956-.159-.37-.212-1.163-.423-1.428-.37.529-1.428 1.64-2.326 1.48-.16-.21-.318-.422-.423-.687-.212-.687-.106-1.797.105-2.432.37-1.322.74-2.273 1.428-3.225.211-.317.529-.529.74-.846-.423-.423-.317-1.692-.053-2.22.159-.37.423-.318.476-.9-.106-.158 0-.37-.053-.581-.211-.952.159-2.115.423-2.855.9-2.38 2.432-4.072 4.706-5.023.529-.212 1.163-.37 1.745-.476.264-.053.528 0 .74-.106 5.34-.053 8.037 2.75 9.041 7.032.159.688 0 1.586 0 2.274 1.216.264 1.48 2.696.476 3.384V23c.37.634.846 1.269 1.163 1.956.318.74.423 1.586.635 2.485.211.687-.106 2.274-.423 2.485z" fill="#18acfc"></path></svg></a>
              <a href=""><svg viewBox="0 0 46 46" xmlns="http://www.w3.org/2000/svg" aria-label="weibo"><g fill="#ea5d5c"><path d="M20.156 21.662c-4.35.21-7.862 2.55-7.862 5.478s3.513 5.144 7.862 4.935c4.35-.21 7.862-2.97 7.862-5.897-.042-2.885-3.555-4.725-7.862-4.516zm2.927 7.402c-1.338 1.756-3.972 2.593-6.523 1.17-1.213-.668-1.171-1.965-1.171-1.965s-.502-4.14 3.847-4.642c4.391-.543 5.186 3.68 3.847 5.437z"></path><path d="M20.156 26.262a.613.613 0 00-.167.836c.125.251.502.293.752.084.251-.21.377-.585.21-.836-.126-.251-.46-.293-.795-.084zm-2.05.585a1.53 1.53 0 00-1.38 1.506c0 .67.67 1.17 1.465 1.087.794-.083 1.463-.71 1.463-1.422s-.627-1.254-1.547-1.17z"></path><path d="M23 0C10.287 0 0 10.287 0 23s10.287 23 23 23 23-10.287 23-23S35.713 0 23 0zm8.866 28.144c-1.799 3.847-7.695 5.73-12.044 5.394-4.14-.334-9.493-1.714-10.036-6.816 0 0-.293-2.3 1.923-5.269 0 0 3.178-4.475 6.858-5.77 3.722-1.255 4.14.877 4.14 2.174-.209 1.087-.543 1.714.837 1.296 0 0 3.638-1.715 5.143-.21 1.213 1.214.21 2.928.21 2.928s-.502.544.543.753c.962.167 4.224 1.673 2.425 5.52zm-3.597-10.622a.733.733 0 01-.71-.71c0-.42.334-.712.71-.712 0 0 4.475-.836 3.931 4.015v.083a.723.723 0 01-.71.628.705.705 0 01-.712-.711c0-.084.711-3.304-2.509-2.593zm7.57 4.642c-.126.836-.544.502-1.004.502-.586 0-1.046-.753-1.046-1.339 0-.501.21-1.003.21-1.003.041-.21.543-1.547-.335-3.513-1.59-2.718-4.768-2.718-5.144-2.593-.376.168-.962.251-.962.251-.585 0-1.045-.46-1.045-1.045 0-.502.334-.878.752-1.004 0 0 0 .042.042.042s.084.042.084.042c.46-.084 2.049-.21 3.554.167 2.802.67 6.566 3.764 4.893 9.493z"></path></g></svg></a>
            </div>
          </div>
        </div>
      </div>
      <!-- 登录 end-->
      <!-- 版权 -->
      <div class="copyright">
        <span>小米公司版权所有-京ICP备10046444-京公网安备11010802020134号-京ICP证110507号</span>
      </div>
       <!-- 版权 end-->
    </div>
  </div>
</template>

<script setup>
import { ref } from 'vue';
import { useRouter } from 'vue-router';
const router = useRouter();


const username = ref('');
const password = ref('');

const registerUser = () => {
  if (!username.value || !password.value) {
    alert('用户名或密码不能为空！');
    return;
  }
  // 从 localStorage 获取用户数据
  const storedUser = localStorage.getItem("userinfo");
  if (!storedUser) {
    alert('用户不存在，请先注册！');
    // 跳转到注册页面
    router.push('/register')
    return;
  }

  const userData = JSON.parse(storedUser);
  if(userData.username != username.value){
    alert("用户名错误")
    return;
  }
  if (userData.password !== password.value) {
    alert("密码错误！");
    return;
  }
  
  // 模拟注册逻辑（实际应用中需要发送请求到后端）
  alert("登陆成功，0.5秒后跳转到官网页");
  
  // 清空输入框
  username.value = '';
  password.value = '';

  // 登陆成功后跳转到官网页
  router.push('/')
};
</script>
<style scoped>
/* 大盒子 */
.box{
  /* 让元素 覆盖整个屏幕 */
  position: fixed;
  width: 100%;
  height: 100vh;
  /* flex布局，横向排放 */
  display: flex;
  flex-direction: row;
  background-color: rgb(255, 255, 252);
}
/* 左边图片 */
.box .img img{
  height: 100%;
  width: 100%;
  /* border: 1px solid red; */
}
/* 左边图片 end */
/* 右边部分 */
.box .two{
  width: 100%;
  /* flex布局，纵向排列 */
  display: flex;
  flex-direction: column;
}
/* 导航 */
.box .TwoNavigation{
  width: 100%;
  height: 80px;
  /* background-color: #e69393; */
  /* flex布局 */
  display: flex;
  justify-content: space-between;/* 让盒子中各元素平分空间 */
  position: relative; /* 让 .sub-nav 以这个元素为基准定位 */
}
/* loge */
.box .loge{
  width: 100px;
  height: 100%;
  /* background-color: #b6f781; */
  /* flex布局， */
  display: flex;
  justify-content: center;
  align-items: center;
}
.box .loge img{
  width: 50px;
  height: 50px;
  border-radius: 20px;/* 图标边框圆角 */
}
/* loge end*/
/* right */
.box .TwoNavigation .right ul{
  list-style: none;/* 去掉li标签的圆点 */
  background-color: #fff;
  margin-right: 20px;/* ul整体与右边的外边距 */
}
.box .TwoNavigation .right ul li{
  height: 50px;/* 每个li的高度 */
  float: left;/* 各个li标签横向排放 */
  margin-left: 20px;/* 每个li标签与左边的距离 */
  color: rgb(182, 182, 179);
  /* background-color: #fff; */
  position: relative; /* 让 .sub-nav 以这个元素为基准定位 */
}
.box .TwoNavigation .right ul li a{
  display: block;/* 设置每个a标签为块级元素 */
  color: rgb(182, 182, 179);
  background-color: #fff;
  text-decoration: none;/* 去掉a标签的下划线 */
}
/* 鼠标经过a标签时 */
.box .TwoNavigation .right ul li a:hover{
  color: black;
}
/* right end*/
/* 二级导航样式 */
.sub-nav {
  position: absolute;/* 绝对定位 */
  left: -20px;  /* 让子菜单和父级导航项左侧对齐 */
  top: 100%; /* 让子菜单紧贴主导航的底部 */
  background-color: #fff;
  border: 1px solid rgb(182, 182, 179);/* 二级导航的边框 */
  padding: 1px;/* 二级导航的内边距 */
  box-shadow:1px 5px 20px -2px rgb(197, 196, 196);/* 边框阴影 */
  display: none;/* 隐藏项目 */
}
/* 鼠标悬停时显示二级导航 */
.box .TwoNavigation .right li:hover .sub-nav {
  margin-left: 1px;/* 控制二级导航的外边距，保证与主导航并列 */
  display: block;/* 显示项目 */
  font-size: smaller;
}
/* 二级导航的li标签属性 */
.box .TwoNavigation .right li:hover .sub-nav li{
  /* li标签的内外边距为零 */
  padding: 0;
  margin: 0;
  /* li标签的长高、文字居中、行高 */
  width: 110px;
  height: 30px;
  text-align: center;
  line-height: 30px;
}
/* 鼠标经过二级导航的li标签时背景颜色 */
ul li .sub-nav li:hover{
background-color: #f1c1c1;
}
/* 二级导航样式 end */
/* 导航 end */

/* 登录 */
.box .form{
  height: 100%;
  display: flex;
  justify-content: center;
  align-items: center;
  /* border: 1px solid rgb(182, 182, 179); */
}
.box .form .form-center{
  width: 350px;
  background-color: #fff;
  box-shadow:1px 20px 20px -2px rgb(175, 174, 174);/* 边框阴影 */
  padding: 50px;/* 内边距 */
}
.box .form .form-center .title{
  color: black;
  font-weight: bold;
  font-size: 25px;
  text-decoration: none;
}
.box .form .form-center .title:hover{
  color: red;
}
.box .form .form-center .input{
  width: 100%;
  height: 50px;
  margin-top: 20px;
  background-color: rgba(247, 250, 241, 0.842);
  border: none;
}
.box .form .form-center .tk{
  margin-top: 20px;
}
.box .form .form-center .button{
  width: 100%;
  height: 50px;
  color: #fff;
  margin-top: 20px;
  font-size: 20px;
  border: none;
  background-color: #f5b641;
}
.box .form .form-center .button:hover{
  background-color: #f89a1f;
  cursor: pointer; 
}
.box .form .form-center .lj{
  display: flex;
  justify-content: space-between;
  margin-top: 20px;
}
.box .form .form-center .lj a{
  font-size: 18px;
  text-decoration: none;
}
.box .form .form-center .qtfsdl{
  width: 100%;
  margin-top: 20px;
  text-align: center;
  display: flex;
  flex-direction: column;
}
.box .form .form-center div svg{
  width: 40px;
  height: 40px;
  margin-left: 10px;
  margin-top: 20px;
}
/* 登录 end */
/* 版权 */
.copyright{
  text-align: center;
  font-size: 15px;
  margin-bottom: 50px;
}
/* 版权 end */
/* 右边部分 end */
</style>